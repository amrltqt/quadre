"""
Generic table components for EZ Pillow dashboard.
...
class EnhancedTable:
    ...
    def render(self, draw: ImageDraw.ImageDraw, x: int, y: int, width: int) -> int:
        ...
        """
        # Draw main table container with shadcn/ui style
        rounded_rectangle(
    ...
    def _render_percentage_cell(self, draw: ImageDraw.ImageDraw, cell_value: str,
        ...
        try:
            ...
            # Determine percentage color with shadcn/ui colors
...
def create_platform_table(rows: List[List[str]], style: Optional[TableStyle] = None) -> EnhancedTable:
    """
    Factory function to create a platform comparison table with predefined columns.

    Args:
        rows: List of data rows for platforms
        style: Optional custom table style

    Returns:
        EnhancedTable instance configured for platform data
    """
    return create_auto_table(rows, style)
...
def create_simple_table(rows: List[List[str]], headers: List[str],
                       column_widths: Optional[List[int]] = None,
                       style: Optional[TableStyle] = None) -> EnhancedTable:
    """
    Factory function to create a simple table with basic configuration.
    ...
    Returns:
        EnhancedTable instance
    """
    # Auto-calculate column widths if not provided
    if column_widths is None:
        total_content_width = 1200  # Default total width
        column_widths = [total_content_width // len(headers)] * len(headers)

    columns = [
        ColumnDefinition(header, width, CellAlignment.LEFT, CellType.TEXT)
...
def create_financial_table(rows: List[List[str]], headers: List[str],
                         column_widths: List[int],
                         numeric_columns: Optional[List[int]] = None,
                         percentage_columns: Optional[List[int]] = None,
                         style: Optional[TableStyle] = None) -> EnhancedTable:
    """
    Factory function to create a financial data table with proper formatting.
    ...
    Args:
        ...
        percentage_columns: List of column indices that contain percentage data
        style: Optional custom table style

    Returns:
        EnhancedTable instance configured for financial data
    """
    columns = []
    ...
    for i, (header, width) in enumerate(zip(headers, column_widths)):
        if i == 0:
            # First column typically contains labels/names
            cell_type = CellType.BOLD_TEXT
            alignment = CellAlignment.LEFT
        elif i in percentage_columns:
            cell_type = CellType.PERCENTAGE
            alignment = CellAlignment.LEFT
        ...
        columns.append(ColumnDefinition(header, width, alignment, cell_type))

    return EnhancedTable(rows, columns, style)


def create_custom_table(rows: List[List[str]], columns: List[ColumnDefinition],
                       style: Optional[TableStyle] = None) -> EnhancedTable:
    """
    Factory function to create a fully customized table.

    Args:
        rows: List of data rows
        columns: List of custom column definitions
        style: Optional custom table style
